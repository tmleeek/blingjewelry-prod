...
class AvroIOTypeException extends AvroException
{
     ...
     * @param mixed $datum
     */
    public function __construct($expected_schema, $datum, $history = array())
    {
        parent::__construct(sprintf("The datum %s is not an example of schema %s. \n History: %s\n",
            var_export($datum, true), $expected_schema, var_export($history, true)));
    }
}
...
class AvroIODatumWriter
{
    ...
    {
        $valid = AvroSchema::is_valid_datum($writers_schema, $datum, true);

        if(!$valid) {
            $d = var_export($datum);
            Mage::log("datum: {$d}", null, "avro.log", true);
        }
        Mage::log("valid: {$valid}", null, "avro.log", true);
    ...
    function write($datum, $encoder)
    {
        ...
        $d = var_export($datum, true);
        Mage::log("datum: {$d}", null, "avro.log", true);
        $this->write_data($this->writers_schema, $datum, $encoder);
        Mage::log("done writing", null, "avro.log", true);
    }
