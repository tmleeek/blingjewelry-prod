...
class AvroSchema
{
    ...
    public static function is_valid_datum($expected_schema, $datum, $throw_exception = false, $history = array())
    {
        ...
        switch($expected_schema->type)
        {
            case self::NULL_TYPE:
                $valid = is_null($datum);
                if($throw_exception && !$valid) { throw new AvroIOTypeException($expected_schema, $datum, $history);}
                return $valid;
            ...
            case self::LONG_TYPE:
                if (Avro::uses_gmp()){
                    ...
                    if($throw_exception) { throw new AvroIOTypeException($expected_schema, $datum, $history);}
                    return false;
                }
                return (is_int($datum)
            ...
            case self::MAP_SCHEMA:
                if (is_array($datum))
                {
                    foreach ($datum as $k => $v) {
                        ...
                        if (!$valid
                            || !self::is_valid_datum($expected_schema->values(), $v, $throw_exception, $history))
                            return false;
                        array_pop($history);
                    }
                    return true;
            ...
            case self::REQUEST_SCHEMA:
                if (is_array($datum))
                {
                    foreach ($expected_schema->fields() as $field) {
                        ...
                        $history[] = $field->name();
                        if (!$valid
                            || !self::is_valid_datum($field->type(), $datum[$field->name()], $throw_exception, $history))
                            return false;
                        array_pop($history);
                    }
                    return true;
                }
                Mage::log("not array record schema", null, "avro.log", true);
                if($throw_exception) { throw new AvroIOTypeException($expected_schema, $datum, $history);}
                return false;
            default:
                throw new AvroSchemaParseException(
