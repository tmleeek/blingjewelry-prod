 ...
 */
class Bootstrap_Customwidgets_Block_Widget_Productsbyid extends Bootstrap_Customwidgets_Block_Productsbyid
    implements Mage_Widget_Block_Interface
{
    ...
    const DISPLAY_TYPE_ID_PRODUCTS          = 'id_products';
    const DISPLAY_TYPE_CAT_PRODUCTS          = 'cat_products';
    ...
    protected function _getProductCollection()
    {
        switch ($this->getDisplayType()) {
            case self::DISPLAY_TYPE_NEW_PRODUCTS:
                ...
                break;
            case self::DISPLAY_TYPE_ID_PRODUCTS:
                $collection = $this->_getProductCollectionById();
                break;
            case self::DISPLAY_TYPE_CAT_PRODUCTS:
                $collection = $this->_getProductCollectionByCat();
                break;
            default:
     ...
     * @return Mage_Catalog_Model_Resource_Product_Collection|Object|Varien_Data_Collection
     */
    protected function _getProductCollectionById()
    {
        /** @var $collection Mage_Catalog_Model_Resource_Product_Collection */
        $collection = Mage::getResourceModel('catalog/product_collection');
        $collection->setVisibility(Mage::getSingleton('catalog/product_visibility')->getVisibleInCatalogIds());
        ...
        $collection = $this->_addProductAttributesAndPrices($collection)
            ->addStoreFilter()
            //->addAttributeToFilter('entity_id', array('from' => "1", 'to' => "500") )
            //->addAttributeToFilter('entity_id', array('in' => array("877","417")))
            ->addAttributeToFilter('entity_id', array('in' => explode(",",$this->getProductIds())))
            ->addAttributeToSort('created_at', 'desc')
            ->setPageSize($this->getProductsCount())
            ->setCurPage(1)
        ;
     ...
     * @return Mage_Catalog_Model_Resource_Product_Collection|Object|Varien_Data_Collection
     */
    protected function _getProductCollectionByCat()
    {
        $category = Mage::getModel('catalog/category')->load($this->getCatId()); 
        /** @var $collection Mage_Catalog_Model_Resource_Product_Collection */
        $collection = Mage::getResourceModel('catalog/product_collection');
        $collection->setVisibility(Mage::getSingleton('catalog/product_visibility')->getVisibleInCatalogIds());
    ...
    {
        if (!$this->hasData('cat_id')) {
            return "";
        }
        return intval($this->getData('cat_id'));
    }
