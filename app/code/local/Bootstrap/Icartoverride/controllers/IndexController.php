...
/* 
    mmc notes
    changed all wishlist references to favorites
    changed shopping cart to shopping bag
    bling added logic to use ShortTitle if present
*/
require_once('Mage/Checkout/controllers/CartController.php');
class Bootstrap_Icartoverride_IndexController extends Mage_Checkout_CartController
{
        ...
        try {
            ...
            if (!$this->_getSession()->getNoCartRedirect(true)) {
                if (!$cart->getQuote()->getHasError()){
                    ...
                    if(!empty($shortTitle)){
                        $_productName = Mage::helper('core')->htmlEscape($shortTitle);
                    }else{
                        $_productName = Mage::helper('core')->htmlEscape($product->getName());
                    }
                    // end

                    $message = Mage::helper('checkout')->__('%s was added to your shopping bag.', $_productName);
            ...
            $this->_getSession()->addException($e, Mage::helper('checkout')->__('Cannot add the item to shopping bag.'));
            $this->getRequest()->setParam('error', 1);
        }
        $this->_forward('added');
    }
    ...
    public function editAction() {                
        ...
        try {
            ...
            if (!$this->_getSession()->getNoCartRedirect(true)) {                
                ...
                if (!$cart->getQuote()->getHasError()) {
                    if ($isRemoveItemFlag) {
                        $message = Mage::helper('icart')->__('%s was deleted form your shopping bag.', Mage::helper('core')->htmlEscape($quoteItem->getName()));
                    } else {
                        $message = Mage::helper('checkout')->__('%s was updated in your shopping bag.', Mage::helper('core')->htmlEscape($item->getProduct()->getName()));
                    }
                    $this->_getSession()->addSuccess($message);
                }
                //$this->_goBack();
    ...
    protected function _getWishlist()
    {
        try {
            ...
            Mage::getSingleton('wishlist/session')->addException($e,
                Mage::helper('wishlist')->__('Cannot create favorites list.')
            );
            return false;
        }
        return $wishlist;
    ...
    {                       
        ...
        try {
            ...
            // end            
            $session->addSuccess($this->__('%1$s was successfully added to your favorites.', $_productName));            
            ...
            $session->addException($e, $this->__('Cannot add the item to favorites.'));
        }
    ...
    protected function _updateShoppingCart()
    {
        try {
            ...
            $this->_getSession()->addException($e, $this->__('Cannot update shopping bag.'));
            //Mage::logException($e);
        }
    }
