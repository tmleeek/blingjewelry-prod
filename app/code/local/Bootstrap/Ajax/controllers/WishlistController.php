<?php
class Bootstrap_Ajax_WishlistController extends Mage_Core_Controller_Front_Action
{
	...
	public function compareAction(){
		...
		if ($productId = (int) $this->getRequest()->getParam('product')) {
			...
			if ($product->getId()/* && !$product->isSuper()*/) {
				...
				$sidebar_block->setTemplate('bootstrap/ajax/catalog/product/compare/sidebar.phtml');
				...
				if (!$productId) {
					...
					if (!$product->getId() || !$product->isVisibleInCatalog()) {
						...
						try {
							...
							// bling added to account for short_title
							if($product->getShortTitle()){
								$_productName = $product->getShortTitle();
							}else{
								$_productName = $product->getName();
							}
							$message = $this->__('%1$s has been added to your wishlist.', $_productName, $referer);
							$response['status'] = 'SUCCESS';
							...
							// todo: turn back on for generic theme
							//$this->loadLayout();
