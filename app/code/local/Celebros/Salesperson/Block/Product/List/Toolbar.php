...
class Celebros_Salesperson_Block_Product_List_Toolbar extends Mage_Core_Block_Template
{
	...
	public function getDefaultDirection($key)
	{
		switch($key)
		{
			case 'mysort':
    ...
    public function getAvailableOrders()
    {
		...
		$this->_availableOrder = array('relevancy' => 'Relevancy', 'AgeRank' => 'New Arrivals', 'name' => 'Name', 'price' => 'Price');
		$this->setDefaultOrder('AgeRank');
		//echo "<!-- " . print_r($this->_availableOrder, true) . " -->";
    ...
    {
    	$lastNum = $this->getFirstNum() + $this->getQwiserSearchResults()->SearchInformation->PageSize - 1;
    	...
    	$lastNum = ($lastNum < $this->getTotalNum()) ? $lastNum : $this->getTotalNum();
    ...
    {
        $pagerBlock = Mage::getBlockSingleton('salesperson/product_list_toolbar_pager');
